...
caching:Cache initiatedTransactions = caching:createCache("transactions.initiated.cache", 600000, 10, 0.1);

documentation {
    This cache is used for caching transaction that are this Ballerina instance participates in.
}
caching:Cache participatedTransactions = caching:createCache("transactions.participated.cache", 600000, 10, 0.1);

documentation {
    This cache is used for caching HTTP connectors against the URL, since creating connectors is expensive.
}
caching:Cache httpClientCache = caching:createCache("http.client.cache", 900000, 10, 0.1);
